...
- SDK 的 HTTP 日志只输出脱敏 URL、方法、状态码、请求 ID 和字节数，不输出请求体或响应体；Token、OAuth 凭据、邀请码和邀请 URL 会被脱敏。
- Webhook 必须配置非空 verify token；无效旧构造器会产生失败关闭实例。
- Webhook 默认在处理器正常返回后才确认事件；多实例应实现事务型共享去重器，并保持业务操作幂等。
- Encrypt Key 最长 32 字节；较短 Key 会按 KOOK 算法补零，不应为此手工改写平台生成的 Key。
